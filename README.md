...
| [docs/deploy.md](docs/deploy.md) | 部署说明（Pages + Worker + D1） |
...
build/functions/        # Pages Functions：/api/contact、/api/newsletter（同域表单接口）
worker.js               # 备选：独立 Worker 版表单接口（如 Pages Functions 不可用）
database/init.sql       # D1 建表（leads / subscribers）
_headers                # 安全响应头
.github/workflows/deploy.yml  # 推送 master 自动部署到 Cloudflare Pages
```
...
- **已交付**：双语官网 20 页（含隐私/条款/退款政策）+ 语言跳转 + Pages Functions 表单接口 + 首篇中文博客 + 部署配置
...
纯静态 HTML/CSS/JS（零构建依赖）+ Cloudflare Pages 托管 + Worker 表单接口 + D1 存储。
